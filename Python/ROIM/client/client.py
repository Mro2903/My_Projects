...
class ClientApp(MDApp):
    ...
    def get_viewers(self, label_id='Stream_video'):
        while self.capture or len(self.root.ids["Watch_layout"].children) == 3:
            time.sleep(5)
...
if __name__ == '__main__':
    ...
    if connected:
        ...
        app.stop_stream()
        if len(app.root.ids["Watch_layout"].children) == 3:
            app.root.ids["Watch_layout"].children[0].stop = True
            app.root.ids["Watch_layout"].children[0].sock.close()
        sock.close()
